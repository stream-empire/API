...
const getUsers = (request, response) => {
        streamer.findAll().then(streamers => {
            response.status(200).json(streamers.map(s => JSON.parse(`{"discordName": ${s.discordName ? `"${s.discordName}"` : null}, "discordId": ${s.discordId ? `"${s.discordId}"` : null}, "twitchName": ${s.twitchName ? `"${s.twitchName}"` : null}, "twitchId": ${s.twitchId ? `"${s.twitchId}"` : null}, "siteName": "${s.siteName}", "shards": ${s.shards}, "thumbnail": ${s.thumbnail ? `"${s.thumbnail}"` : null}, "tier": "${s.tier}", "admin": ${s.admin}}`)));
        }, err => {
...
const getUser = (request, response) => {
    ...
    if (request.query.discordid || request.query.sitename || request.query.twitchname) {
        ...
        streamer.findAll({
            ...
            else response.status(200).json(JSON.parse(`{"discordName": ${streamers[0].discordName ? `"${streamers[0].discordName}"` : null}, "discordId": ${streamers[0].discordId ? `"${streamers[0].discordId}"` : null}, "twitchName": ${streamers[0].twitchName ? `"${streamers[0].twitchName}"` : null}, "twitchId": ${streamers[0].twitchId ? `"${streamers[0].twitchId}"` : null}, "siteName": "${streamers[0].siteName}", "shards": ${streamers[0].shards}, "thumbnail": ${streamers[0].thumbnail ? `"${streamers[0].thumbnail}"` : null}, "tier": "${streamers[0].tier}", "admin": ${streamers[0].admin}}`));
        }, err => {
            response.status(500).json({error: err, status: 'Internal Server Error'})
        })
    }
    ...
    streamer.findAll({
        ...
        if (streamers[0] === undefined) {
            ...
            bcrypt.hash(password, 10).then(hashedPass => {
                streamer.create({siteName: name, email: email, verified: false, createdAt: now, updatedAt: now, password: hashedPass, shards: 0, tier: 'basic', admin: false}).then(newStreamer => {
